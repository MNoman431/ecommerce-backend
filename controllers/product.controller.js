...
// ✅ Add Product
export const addProduct = asyncHandler(async (req, res) => {
  const { name, description, price, stock, category, carModel, color, material } = req.body;
  ...
  const product = await Product.create({
    ...
    color,
    material,
    imageUrl: uploadedImages[0], // first image as main image
...
export const updateProduct = asyncHandler(async (req, res) => {
  ...
  // Agar nayi images aayi hain to upload karo
  if (req.files && req.files.length > 0) {
    const uploadedImages = await Promise.all(
      req.files.map((file) => {
        return new Promise((resolve, reject) => {
    ...
    imageUrl = uploadedImages[0]; // first image as main
  }
